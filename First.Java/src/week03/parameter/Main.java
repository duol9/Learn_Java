...
public class Main {
    public static void main(String[] args) {
        ...
        // 메서드 실행 완료 후 전달할 매개값으로 저장된 type 값 확인
        System.out.println("type = " + type);  // 기존에 선언한 'D' 출력
        // 메서드 실행 완료 후 반환된 car 인스턴스의 gear 타입 확인
        System.out.println("gear = " + car.gear);  // 객체 내부에서 type 변경하여 수정했기 때문에 'P' 출력

        System.out.println();

        // 참조할 매개변수
        Tire tire = new Tire();
        tire.company = "금호";  // 금호 타이어객체 생성

        // 차 객체의 타이틀을 등록하는 매서드 호출한 후 반환값으로 차 객체의 타이어 객체 반환
        Tire carInstanceTire = car.setTire(tire);

        // 메서드 실행 완료 후 전달할 매개값으로 지정된 참조형 변수 tire의 company 값 확인
        System.out.println("tire.company = " + tire.company); // "KIA" 춣력
